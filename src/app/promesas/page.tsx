...
import { promesasPR } from '@/lib/data/oficial-pr'
...
function PromiseCard({ promise }: { promise: typeof promesasPR[0] }) {
...
export default function PromesasPage() {
  ...
  const filtradas = filtro === 'todas'
    ? promesasPR
    : promesasPR.filter(p => p.estado === filtro)

  return (
    <PageShell title="Rastreador de Promesas" subtitle="Compromisos públicos verificados">
      <div className="py-4">
        {/* Summary pills */}
        <div className="px-4 mb-4">
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Total', val: promesasPR.length, cls: 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' },
              { label: 'Completadas', val: promesasPR.filter(p => p.estado === 'completada').length, cls: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' },
              { label: 'En Progreso', val: promesasPR.filter(p => p.estado === 'en_progreso').length, cls: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' },
            ].map(({ label, val, cls }) => (
        ...
        <p className="text-center text-[10px] text-gray-400 dark:text-gray-600 mt-4 px-4">
          Fuente: Plataforma Electoral PNP 2024 y registros oficiales del Gobierno de Puerto Rico
        </p>
      </div>
    </PageShell>
  )
