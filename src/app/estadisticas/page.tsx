...
const categorias = ['Todas', ...Array.from(new Set(estadisticasPR.map(s => s.categoria)))]

function StatChart({ stat }: { stat: typeof estadisticasPR[0] }) {
...
export default function EstadisticasPage() {
  ...
  const filtradas = categoriaActiva === 'Todas'
    ? estadisticasPR
    : estadisticasPR.filter(s => s.categoria === categoriaActiva)

  return (
    <PageShell title="Estadísticas Oficiales" subtitle="Datos de agencias gubernamentales">
      <div className="py-4">
        {/* Category filter */}
        ...
        <div className="mx-4 mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
          <p className="text-[10px] text-gray-500 dark:text-gray-400 text-center">
            Todos los datos provienen de agencias oficiales del Gobierno de Puerto Rico.
            Los gráficos reflejan datos históricos verificados. Fuentes: DTRH, PREB, ASES, COR3, Compañía de Turismo, Junta de Planificación.
          </p>
        </div>
      </div>
    </PageShell>
